...
exports.getApplications = async (req, res) => {
    try {
        ...
        let applications = await VendorApplication.find(query)
            .populate("category")
            .sort("-createdAt")
            .skip(skip)
            .limit(parseInt(limit));

        // Field Level Security - Strip sensitive data if only basic view exists
        ...
        if (!adminPerms.includes("APPLICATION_VIEW_TECHNICAL") &&
            !adminPerms.includes("APPLICATION_VIEW_FINANCIAL") &&
            !adminPerms.includes("APPLICATION_VIEW_COMPLIANCE") &&
            !isSuper) {

            applications = applications.map(app => {
                const appObj = app.toObject();
                // Redact sensitive data from the list view
                delete appObj.data;
        ...
        return successResponse(res, "Applications retrieved", applications, 200, pagination);
    } catch (err) {
