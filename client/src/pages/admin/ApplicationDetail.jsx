...
import {
    ...
    AlertTriangle, ThumbsUp, ThumbsDown, X, Layout, CheckCircle2,
    User, Calendar
} from "lucide-react";
...
export default function ApplicationDetail({ app, onSuccess }) {
    ...
    // ── NEW: Direct Decision for Super Admins (Bypass Workflow) ──
    const handleDirectApprove = async () => {
        if (!window.confirm("You are about to ACTIVATE this vendor immediately, MISSION CRITICAL decision. Proceed?")) return;
        setProcessing(true);
        const toastId = toast.loading("Executing final authorization...");
        try {
            const res = await api.post(`/applications/${appData._id}/approve`, {
                remarks: remarks.trim() || "Immediate system authorization by Super Admin."
            });
            if (res.data?.data) setAppData(res.data.data);
            toast.success("✅ VENDOR ACTIVATED & ACCOUNT PROVISIONED", { id: toastId });
            await refreshApp();
            onSuccess();
        } catch (err) {
            toast.error(err.response?.data?.message || "Direct approval failed", { id: toastId });
        } finally {
            setProcessing(false);
        }
    };
    ...
    const handleDirectReject = async () => {
        ...
        setProcessing(true);
        try {
            const res = await api.post(`/applications/${appData._id}/reject`, {
                remarks: rejectReason.trim()
            });
            if (res.data?.data) setAppData(res.data.data);
            toast.success("Application officially rejected.");
            setShowRejectModal(false);
            setRejectReason("");
            await refreshApp();
            onSuccess();
    ...
    const tabs = appData.isTree ? [
        { id: "dossier", label: "Dossier Detail", icon: Layout },
    ...
    if (appData.isTree) {
        return (
            <div className="flex flex-col gap-6 p-2">
                {/* Header mimicking Page Header */}
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                        <StatusBadge status={appData.status} />
                        <div>
                            <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">{appData.formName || "Entry Dossier"}</h2>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                <User size={10} /> {appData.vendorName} • <Mail size={10} /> {appData.vendorEmail}
                            </p>
                        </div>
                    </div>
                </div>

                {/* Full-width High Density Sections */}
                <div className="space-y-6">
                    {Object.entries(appData.data?.reduce((acc, item) => {
                        const path = item.nodePath || "General Information";
                        if (!acc[path]) acc[path] = [];
                        acc[path].push(item);
                        return acc;
                    }, {}) || {}).map(([path, items]) => (
                        <div key={path} className="border border-slate-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all bg-white">
                            <div className="bg-slate-50/80 px-5 py-3 border-b border-slate-100 flex items-center gap-3">
                                ...
                                <h2 className="text-[12px] font-bold text-slate-800 tracking-wide uppercase">{path}</h2>
                            </div>
                            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-6">
                                {items.map((item, idx) => (
                                    <div key={idx} className="flex flex-col gap-1.5 group">
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none group-hover:text-indigo-500 transition-colors">{item.label}</p>
                                        {item.type === "file" ? (
                                            <div className="mt-1">
                                                {item.fileUrl ? (
                                                    <a href={`${(import.meta.env.VITE_API_URL || "http://localhost:5000/api").replace("/api", "")}${item.fileUrl}`} className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[12px] font-bold text-slate-700 hover:bg-white hover:border-indigo-200 hover:text-indigo-600 transition-all shadow-sm" target="_blank" rel="noreferrer">
                                                        <FileText size={14} /> {item.fileName || "View Exhibit"}
                                                    </a>
                                                ) : <span className="text-[12px] text-slate-300 italic font-medium">Not provided</span>}
                                            </div>
                                        ) : (
                                            <p className="text-[14px] font-bold text-slate-900 break-words leading-relaxed">{String(item.value || "-")}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
                ...
                {/* Decision Interface at the bottom */}
                <div className="mt-4 border-t border-slate-100 pt-8">
                    <div className="flex flex-col gap-4">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Administrative Authorization Remarks</label>
                        <textarea
                            value={remarks}
                            onChange={(e) => setRemarks(e.target.value)}
                            className="w-full border border-slate-200 rounded-xl p-4 text-sm font-medium focus:ring-4 focus:ring-indigo-50 focus:border-indigo-300 outline-none transition-all placeholder-slate-300 min-h-[100px]"
                            placeholder="Enter review notes, compliance observations, or authorization details..."
                        />
                        <div className="flex gap-4">
                            <button
                                onClick={() => setShowRejectModal(true)}
                                disabled={processing}
                                className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-rose-50 hover:text-rose-600 transition-all active:scale-95 border border-transparent hover:border-rose-100"
                            >
                                Reject Submission
                            </button>
                            <button
                                ...
                                className="flex-[2] py-4 bg-slate-900 text-white rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-emerald-600 transition-all active:scale-95 flex items-center justify-center gap-3 shadow-xl shadow-slate-200 disabled:opacity-50"
                            >
                                <CheckCircle2 size={16} />
                                {processing ? "Processing..." : "Approve & Provision Vendor Account"}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex-1 min-w-0">
                <div className="bg-white border border-gray-200 rounded-[8px] shadow-sm flex flex-col h-full min-h-[600px]">
                    ...
                    <div className="p-10 flex-1 overflow-y-auto">
                        ...
                        {(activeTab === "dossier" || !activeTab) && appData.isTree && (
                            <div className="space-y-6">
                                {Object.entries(appData.data?.reduce((acc, item) => {
                                    const path = item.nodePath || "General Information";
                                    if (!acc[path]) acc[path] = [];
                                    ...
                                    <div key={path} className="border border-slate-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                                            ...
                                            {items.map((item, idx) => (
                                                <div key={idx} className="flex flex-col gap-1">
                                                    ...
                                                    {item.type === "file" ? (
                                                        <div className="mt-1">
                                                            {item.fileUrl ? (
                                                                <a href={`${(import.meta.env.VITE_API_URL || "http://localhost:5000/api").replace("/api", "")}${item.fileUrl}`} className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[13px] font-semibold text-slate-700 hover:bg-slate-100 shadow-sm" target="_blank" rel="noreferrer">
                                                                    <FileText size={14} className="text-indigo-600" /> {item.fileName || "View Document"}
                                                                </a>
                                                            ) : <span className="text-[13px] text-slate-300 italic font-medium">No document</span>}
                                                        </div>
                                                    ) : (
                                                        <p className="text-[14px] font-semibold text-slate-900 break-words leading-normal">{String(item.value || "-")}</p>
                                                    )}
                                                </div>
                                            ))}
                                        </div>
                    ...
                    {isApprovedOrRejected ? (
                        ...
                        <div className="p-8 border-t border-gray-200 bg-gray-50/50 flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2">
                                <ShieldCheck size={18} className="text-[#0B5D3B]" />
                                <span className="text-[11px] font-black text-gray-500 uppercase tracking-widest italic">Final Procurement Status Locked</span>
                            </div>
                            <StatusBadge status={appData.status} />
                        </div>
                    ) : canAct ? (
                        /* Pending — show action panel */
                        <div className="p-8 border-t-4 border-[#0B5D3B] bg-white shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
                            {/* Super Admin Control Strip */}
                            {userLevel >= ROLE_HIERARCHY["admin"] && (
                                <div className="mb-6 pb-6 border-b border-dashed border-gray-200 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <div className="h-2 w-2 rounded-full bg-rose-500 animate-pulse"></div>
                                        <span className="text-[10px] font-black text-rose-600 uppercase tracking-[0.2em]">High Privilege Controls — Workflow Bypass Authorized</span>
                                    </div>
                                    <div className="flex gap-2">
                                        <button 
                                            onClick={() => setShowRejectModal(true)}
                                            className="px-4 py-1.5 border border-rose-200 text-rose-500 text-[9px] font-black uppercase rounded-[4px] hover:bg-rose-600 hover:text-white transition-all tracking-widest"
                                        >
                                            Force Reject
                                        </button>
                                        <button 
                                            onClick={handleDirectApprove}
                                            className="px-4 py-1.5 bg-[#0B5D3B] text-white text-[9px] font-black uppercase rounded-[4px] hover:bg-black transition-all tracking-widest shadow-lg shadow-emerald-100"
                                        >
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between items-center mb-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block">
                                        Auditor Remarks <span className="text-rose-400">*</span>
                                    </label>
                                    <span className="text-[9px] font-black text-[#0B5D3B]/40 uppercase tracking-widest">Stage: {appData.currentStage}</span>
                                </div>
                                <textarea
                                    value={remarks}
                                    onChange={(e) => setRemarks(e.target.value)}
                                    className="w-full border border-gray-100 bg-gray-50/30 rounded-xl p-5 text-sm font-medium focus:border-[#0B5D3B] focus:bg-white focus:ring-8 focus:ring-[#0B5D3B]/5 outline-none transition-all placeholder-gray-300 resize-none shadow-inner"
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-4">
                                ...
                                <button
                                    ...
                                    disabled={processing}
                                    className="px-6 py-3 bg-white border-2 border-amber-400 text-amber-600 text-[11px] font-black uppercase tracking-wider rounded-xl hover:bg-amber-400 hover:text-white transition-all flex items-center gap-2 disabled:opacity-40"
                                >
                                    <MessageSquare size={15} /> Clarification Req.
                                </button>
                                ...
                                <div className="flex items-center gap-3 ml-auto">
                                    ...
                                    <button
                                        onClick={() => setShowRejectModal(true)}
                                        disabled={processing}
                                        className="px-7 py-3.5 bg-white border-2 border-rose-100 text-rose-500 text-[11px] font-black uppercase tracking-wider rounded-xl hover:border-rose-500 hover:text-rose-600 transition-all flex items-center gap-2 disabled:opacity-40"
                                    >
                                        <ThumbsDown size={15} /> Disapprove Segment
                                    </button>
                                    ...
                                    <button
                                        onClick={handleApprove}
                                        disabled={processing || !remarks.trim()}
                                        className="px-9 py-3.5 bg-gradient-to-br from-[#0B5D3B] to-[#04472c] text-white text-[11px] font-black uppercase tracking-wider rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-3 shadow-xl shadow-emerald-200/50 disabled:opacity-40 disabled:grayscale"
                                    >
                                        <ThumbsUp size={16} />
                                        {processing ? "Syncing..." : "Authorize Segment"}
                        ...
                        <div className="p-8 border-t border-gray-100 bg-emerald-50/20 flex flex-col items-center justify-center text-center gap-4">
                            <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm border border-emerald-100">
                                <Clock size={20} className="text-emerald-700 animate-pulse" />
                            </div>
                            <div className="max-w-[400px]">
                                <h5 className="text-[11px] font-black text-emerald-900 uppercase tracking-widest mb-1">Queue Synchronization Active</h5>
                                <p className="text-[10px] font-medium text-emerald-600/70 leading-relaxed uppercase">
                                    Current segment <strong>{appData.currentStage}</strong> is pending authorization by <strong>{appData.workflowStages?.find(s => s.stageName === appData.currentStage)?.assignedRole || "Technical Reviewers"}</strong>.
                                </p>
                            </div>
                        </div>
                    )}
